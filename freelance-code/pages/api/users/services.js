...
export default async function getServices(req, res) {
  ...
  if (req.method === "POST") {
    try {
      await db(
        `INSERT INTO services (service_type, description, skills, languages, hourly_rate, resume, github_url, linkedin_url, other_url, images, user_id) VALUES ("${service_type}", "${description}", "${skills}", "${languages}", "${hourly_rate}", "${resume}", "${github_url}", "${linkedin_url}", "${other_url}", "${images}", "${user_id}" );`
      );
